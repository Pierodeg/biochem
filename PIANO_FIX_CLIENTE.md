...
**Stato:** ✅ Fatto (05/07/2026) — progressivo esteso a 4 cifre (`AANNNN`, fino a 9999/anno) e ordinamento per certificazione reso robusto (confronto numerico anno+progressivo invece che testuale). `flutter analyze`: 0 errori sui file toccati.
...
**Stato:** 🔄 In corso — **pilota Preventivo ora in `staging`** (mergiato da `feature/campi-configurabili` il 10/07/2026). Widget `CampoConfigurabile`: cascata a menu automatica sezione→categoria→(sottocategoria)→suggerimenti; binding persistito in `impostazioni/field_bindings`; `defaultCategoriaId` mantiene pre-configurati i campi già a lista; reset admin con conferma + "ripristina predefinita". Migrati i campi Preventivo (oggetto, pagamento, durata, rinnovo, periodo, validità, note). Restano: Servizi lab (tecnico, tipo analisi) e Ondata 2 (campi a testo libero/dropdown chiusi nelle altre pagine).
...
|------|------|-------|------|
| 10/07/2026 | **Riorganizzazione MD + pulizia rami** | 🧹 | Rami `feature/campi-configurabili` e `fix/segnalazioni-cliente` eliminati (ridondanti): resta solo `staging`. Docs consolidati in **3 file**: `STATO_E_ROADMAP.md` (master, ora include "Input dal cliente" §9), questo `PIANO_FIX_CLIENTE.md` (registro tecnico + changelog), `DOMANDE_PER_IL_CLIENTE.md` (ridotto alle sole domande aperte). `INFO_DAL_CLIENTE.md` spostato in `docs/archivio/`. |
| 10/07/2026 | **Merge H1 in staging** | 🔀 | Portato il pilota "campi configurabili" dal branch `feature/campi-configurabili` dentro `staging` (merge no-ff). Ora si lavora su un solo branch; H1 entrerà nel **prossimo deploy** e va testata insieme al resto. Il branch `feature/campi-configurabili` diventa ridondante (da rimuovere). File: `campo_configurabile.dart`, `impostazioni_service.dart`, `preventivo_form_page.dart`. |
| 05/07/2026 | **E4-bis — fix contatore certificazione** | ✅ | Progressivo esteso da 3 a 4 cifre (`AANNNN`, fino a 9999/anno). Aggiunto confronto numerico anno+progressivo per l'ordinamento "N° certificazione" ([filtro_reg_lab.dart](lib/widgets/filtri/filtro_reg_lab.dart)), così l'ordinamento resta corretto anche mescolando vecchi codici a 3 cifre e nuovi a 4. `flutter analyze`: 0 errori (solo info di stile preesistenti). File: [servizi_lab_service.dart](lib/services/servizi_lab_service.dart), [filtro_reg_lab.dart](lib/widgets/filtri/filtro_reg_lab.dart). |
...
| 25/06/2026 | **DEPLOY task cliente** | 🚀 | Deploy per mostrare al cliente le modifiche richieste (aree A–E + fix PDF D2/D3/C6). **H1 accantonato** (ora rientrato in staging il 10/07/2026): il lavoro "campi configurabili" (pilota Preventivo) era preservato sul branch `feature/campi-configurabili`. `staging` riportato a `c48d4de` (solo task cliente). **Post-deploy backlog**: D1 (carta intestata, ora c'è il riferimento), F1, G1 (serve Excel+modello), H1 (riprendere dal branch), H2, H3, H4. |
